...
const CACHE_KEY = "lojas_virais_v1";
...
export default function LojasViraisPage() {
  ...
  const fetchShops = useCallback(async (query: string, force = false) => {
    setLoading(true);
    setError(false);
    ...
    try {
      ...
      if (data.shops && Array.isArray(data.shops)) {
        ...
        if (query.trim().length < 2) {
          sessionStorage.setItem(CACHE_KEY, JSON.stringify({ shops: data.shops, timestamp: Date.now() }));
        }
      } else {
      ...
      <div className="p-4 lg:p-6 space-y-5">
          ...
          <div className="flex gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                onKeyDown={(e) => e.key === "Enter" && fetchShops(search, true)}
              />
            </div>
            <button
              onClick={() => fetchShops(search, true)}
              disabled={loading}
        ...
        {error && !loading && (
          <div className="card border-red-100 bg-red-50 text-center py-8">
            ...
            <button onClick={() => fetchShops(search, true)} className="text-xs text-brand hover:underline">Tentar novamente</button>
          </div>
        )}
        ...
        {!loading && !error && shops.length > 0 && (
          <>
            <div className="flex items-center gap-2 text-sm text-dark-muted">
              ...
              <span>
                <strong className="text-dark">{shops.length}</strong> lojas em alta {search.trim().length >= 2 ? <>no nicho <strong className="text-dark">&ldquo;{search}&rdquo;</strong></> : "selecionadas pela IA agora"}
              </span>
            </div>
            <div className="grid sm:grid-cols-2 gap-4">
              {shops.map((shop, i) => (
